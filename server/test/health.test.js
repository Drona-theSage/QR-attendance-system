...
test("admin can reset password with a valid reset token", async (t) => {
  ...
  const resetResponse = await fetch(
    `http://localhost:${port}/api/auth/reset-password`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
test("admin session response includes a student URL for testing", async (t) => {
  ...
  const subjectResponse = await fetch(
    `http://localhost:${port}/api/admin/subjects`,
    {
      method: "POST",
...
test("subjects are scoped to the admin and persist per course", async (t) => {
  ...
  assert.ok(
    refreshedSubjects.some((subject) => subject.name === "Machine Learning"),
  );
  assert.equal(
    await Subject.countDocuments({
      adminId: (await AdminUser.findOne({ email: "admin@example.com" }))._id,
    }),
    1,
  );
});
...
test("student attendance requires name and class roll number", async (t) => {
  ...
  const createSubject = await fetch(
    `http://localhost:${port}/api/admin/subjects`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", Cookie: adminCookie },
  ...
  const response = await fetch(
    `http://localhost:${port}/api/attendance/${sessionJson.token}/mark`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
test("admin can delete a subject entry", async (t) => {
  ...
  const response = await fetch(
    `http://localhost:${port}/api/admin/subjects/${subject.id}`,
    {
      method: "DELETE",
      headers: { Cookie: adminCookie },
    },
...
test("admin can add a student attendance manually", async (t) => {
  ...
  const sessionJson = await session.json();
  const response = await fetch(
    `http://localhost:${port}/api/admin/sessions/${sessionJson.token}/manual-mark`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", Cookie: adminCookie },
      body: JSON.stringify({
...
test("admin can fetch session history and summary details", async (t) => {
  ...
  const firstSession = await fetch(
    `http://localhost:${port}/api/admin/sessions`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", Cookie: adminCookie },
      body: JSON.stringify({
  ...
  const historyResponse = await fetch(
    `http://localhost:${port}/api/admin/history`,
    {
      headers: { Cookie: adminCookie },
    },
  );
...
test("admin can download attendance after a session expires", async (t) => {
  ...
  const activeResponse = await fetch(
    `http://localhost:${port}/api/admin/sessions/${session.token}/export`,
    {
      headers: { Cookie: adminCookie },
    },
  );
